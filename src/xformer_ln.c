/*
 * xformer_ln.c - Oneiros transformer with RMSNorm (pre-norm), from scratch.
 *
 * Adds the normalization the plain transformer lacked: an RMSNorm before the
 * attention, before the FFN, and before the output head (LLaMA-style pre-norm,
 * simpler than LayerNorm - no mean, no bias). Should train more stably / at a
 * higher LR and reach a lower loss. Hand forward+backprop, `gradcheck`-verified.
 *
 *   r1 = x_{B-1} + Attn( RMSNorm_a(x) )      (residual)
 *   r2 = r1     + FFN ( RMSNorm_f(r1) )       (residual)
 *   logits = Wout tanh( Wh RMSNorm_h(r2) + bh ) + bout
 *
 * Build:  gcc -O2 -march=native -DV=2048 -o xformer_ln src/xformer_ln.c -lm
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
typedef struct {
    float C[V*D], P[B*D];
    float ga[D], gf[D], gh[D];              /* RMSNorm gains (attn/ffn/head) */
...
typedef struct {
    float xt[B][D], xn[B][D], xh_a[B][D]; float irms_a[B];   /* pre-attn RMSNorm */
    float q[D], k[B][D], v[B][D], att[NH][B], ctx[D], attn[D];
    float r1[D], rn[D], xh_f[D]; float irms_f;               /* pre-FFN RMSNorm  */
    float g[FF], r2[D], rh[D], xh_h[D]; float irms_h;        /* pre-head RMSNorm */
    float hid[HID], probs[V];
} Act;
...
/* RMSNorm: y = g * x / rms(x); stores xhat, returns 1/rms */
static float rms_fwd(const float *x, const float *g, float *xhat, float *y){
    float ss=0; for(int i=0;i<D;i++) ss+=x[i]*x[i];
    float irms=1.0f/sqrtf(ss/(float)D+RMS_EPS);
    for(int i=0;i<D;i++){ xhat[i]=x[i]*irms; y[i]=g[i]*xhat[i]; }
    return irms;
}
/* given dy, adds dgain and ACCUMULATES dx */
static void rms_bwd(const float *dy, const float *g, const float *xhat, float irms, float *dx, float *dgain){
    float m=0; for(int i=0;i<D;i++){ dgain[i]+=dy[i]*xhat[i]; m+=dy[i]*g[i]*xhat[i]; }
    m/=(float)D;
    for(int i=0;i<D;i++){ float dxhat=dy[i]*g[i]; dx[i]+=irms*(dxhat - xhat[i]*m); }
}

static void init_params(Params *p){
    for (int i=0;i<V*D;i++) p->C[i]=gauss(0.02f);
    for (int i=0;i<B*D;i++) p->P[i]=gauss(0.02f);
    for (int i=0;i<D;i++){ p->ga[i]=1.0f; p->gf[i]=1.0f; p->gh[i]=1.0f; }
...
static float forward(const Params *p, const sym_t *tok, int target, Act *A){
    ...
    for (int t=0;t<B;t++) A->irms_a[t]=rms_fwd(A->xt[t], p->ga, A->xh_a[t], A->xn[t]);
    for (int a=0;a<D;a++){ float s=0; for(int b=0;b<D;b++) s+=p->Wq[a*D+b]*A->xn[L][b]; A->q[a]=s; }
    for (int t=0;t<B;t++) for (int a=0;a<D;a++){ float ks=0,vs=0;
        for (int b=0;b<D;b++){ ks+=p->Wk[a*D+b]*A->xn[t][b]; vs+=p->Wv[a*D+b]*A->xn[t][b]; }
    ...
    A->irms_f=rms_fwd(A->r1, p->gf, A->xh_f, A->rn);
    for (int j=0;j<FF;j++){ float s=p->bf1[j]; for(int d=0;d<D;d++) s+=p->Wf1[j*D+d]*A->rn[d]; A->g[j]=s; }
    for (int d=0;d<D;d++){ float s=p->bf2[d]; for(int j=0;j<FF;j++){ float r=A->g[j]>0?A->g[j]:0; s+=p->Wf2[d*FF+j]*r; } A->r2[d]=A->r1[d]+s; }
    A->irms_h=rms_fwd(A->r2, p->gh, A->xh_h, A->rh);
    for (int m=0;m<HID;m++){ float z=p->bh[m]; for(int d=0;d<D;d++) z+=p->Wh[m*D+d]*A->rh[d]; A->hid[m]=tanhf(z); }
    float mx=-1e30f;
...
static void backward(const Params *p, const sym_t *tok, int target, const Act *A, Params *g){
    ...
    float drh[D]; for (int d=0;d<D;d++) drh[d]=0;
    for (int m=0;m<HID;m++){ float draw=dhid[m]*(1.0f-A->hid[m]*A->hid[m]); float *gw=g->Wh+m*D; const float *w=p->Wh+m*D;
        for (int d=0;d<D;d++){ gw[d]+=draw*A->rh[d]; drh[d]+=draw*w[d]; } g->bh[m]+=draw; }
    /* RMSNorm head: drh -> g->gh, dr2 */
    float dr2[D]; for (int d=0;d<D;d++) dr2[d]=0;
    rms_bwd(drh, p->gh, A->xh_h, A->irms_h, dr2, g->gh);
    /* r2 = r1 + f */
    ...
    float drn[D]; for (int d=0;d<D;d++) drn[d]=0;
    for (int j=0;j<FF;j++){ g->bf1[j]+=dg[j]; float *gw=g->Wf1+j*D; const float *w=p->Wf1+j*D;
        for (int d=0;d<D;d++){ gw[d]+=dg[j]*A->rn[d]; drn[d]+=dg[j]*w[d]; } }
    /* RMSNorm ffn: drn -> g->gf, dr1 (accumulate) */
    rms_bwd(drn, p->gf, A->xh_f, A->irms_f, dr1, g->gf);
    /* r1 = x_L + attn */
    float dattn[D]; for (int d=0;d<D;d++) dattn[d]=dr1[d];
    float dx[B][D]; for (int t=0;t<B;t++) for (int d=0;d<D;d++) dx[t][d]=0;
    for (int d=0;d<D;d++) dx[L][d]+=dr1[d];
    /* attn = Wo ctx */
    ...
    /* Q/K/V read the normalized xn; accumulate dxn */
    float dxn[B][D]; for (int t=0;t<B;t++) for (int d=0;d<D;d++) dxn[t][d]=0;
    for (int a=0;a<D;a++){ float dqa=dq[a]; float *gw=g->Wq+a*D; const float *w=p->Wq+a*D;
        for (int b=0;b<D;b++){ gw[b]+=dqa*A->xn[L][b]; dxn[L][b]+=dqa*w[b]; } }
    for (int t=0;t<B;t++) for (int a=0;a<D;a++){ float dkta=dk[t][a], dvta=dv[t][a];
        ...
        for (int b=0;b<D;b++){ gwk[b]+=dkta*A->xn[t][b]; dxn[t][b]+=dkta*wk[b];
                               gwv[b]+=dvta*A->xn[t][b]; dxn[t][b]+=dvta*wv[b]; } }
    /* RMSNorm attention: dxn[t] -> g->ga, dx[t] (accumulate) */
    for (int t=0;t<B;t++) rms_bwd(dxn[t], p->ga, A->xh_a[t], A->irms_a[t], dx[t], g->ga);
    /* x_t = C[tok]+P[t] */
...
static void save(const char *path, const Params *p){
    ...
    fwrite("ONEIROS4",1,8,f); fwrite(dims,sizeof(dims),1,f); fwrite(p,sizeof(Params),1,f); fclose(f);
}
static int load(const char *path, Params *p){
    ...
    if (dims[0]!=V||dims[1]!=E||dims[2]!=B||dims[3]!=HID||dims[4]!=NH||dims[5]!=FF){fprintf(stderr,"ckpt dims mismatch\n");fclose(f);return 0;}
...
static void cmd_train(int argc, char **argv){
    ...
    fprintf(stderr,"corpus %zu: train [0,%zu)  params %zu (RMSNorm, D=%d NH=%d FF=%d HID=%d)\n",
            n,train_end,sizeof(Params)/sizeof(float),D,NH,FF,HID);
    double run=0; long runc=0; clock_t t0=clock();
    float base_lr=0.05f; { const char *e=getenv("ONEIROS_LR"); if(e) base_lr=(float)atof(e); }
    long warmup=1000;
